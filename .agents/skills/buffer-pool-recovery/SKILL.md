---
skill: buffer-pool-recovery
status: enforced
severity: medium
applies_to:
  - VisionRemotePS5/Streaming/SafeBufferPool.swift
  - VisionRemotePS5/Streaming/VideoDecoder.swift
  - VisionRemotePS5/Services/StreamingService.swift
evidence:
  - VisionRemotePS5/Services/StreamingService.swift:341-355   # buffer acquire + debounced log
  - TODO.md:46-51   # Phase 1.4 — Buffer Pool Recovery (Anti-Smearing)
---

# Skill: Buffer-Pool Exhaustion Recovery + 1Hz Debounce
...
- Replace `lastBufferExhaustionTime = now` with `lastBufferExhaustionTime = .now` (Date) — would also re-introduce wall-clock dependency (see `skills/monotonic-clock`).
...
5. Change `CACurrentMediaTime()` here to any other clock (covered by `skills/monotonic-clock`).
