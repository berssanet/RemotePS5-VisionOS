...
8. Print to stdout from inside the 120Hz timer block (use `DebugLog` with rate-limit, like the buffer-exhaustion debouncer pattern from the `buffer-pool-recovery` skill).
